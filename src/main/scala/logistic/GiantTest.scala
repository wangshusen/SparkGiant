package distopt.logistic.GiantTest
 ...
 */
class Driver(sc: SparkContext, data: RDD[(Double, Array[Double])], dataTest: RDD[(Double, Array[Double])], isSearch: Boolean = false, isModelAvg: Boolean = false)
    ...
    def train(gamma: Double, maxIter: Int, q: Int): (Array[Double], Array[Double], Array[Double]) = {
        ...
        for (t <- 0 until maxIter) {
            ...
            t1 = System.nanoTime()
            trainErrorArray(t) = testError
            objValArray(t) = this.objVal
            if (!this.isMute) println("Iter " + t.toString + ":\t obj = " + this.objVal.toString + ",\t time = " + timeArray(t).toString + ":\t test err = " + testError.toString)
