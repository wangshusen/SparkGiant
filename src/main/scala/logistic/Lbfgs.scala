...
class Driver(sc: SparkContext, data: RDD[(Double, Array[Double])], isModelAvg: Boolean = false)
    ...
    def twoLoop(): DenseVector[Double] = {
        ...
        for (i <- 0 until k) {
            var j = k - 1 - i
            val aj: Double = (this.pdot(this.sBuffer(j))) / this.syBuffer(j)
            a(j) = aj
            p -= aj * this.yBuffer(j)
        }
        val alpha: Double = this.syBuffer(k-1) / (this.yBuffer(k-1).toArray.map(x => x*x).sum)
        p *= alpha
        for (i <- 0 until k) {
            val bi: Double = (this.pdot(this.yBuffer(i))) / this.syBuffer(i)
