...
object Logistic {  
     ...
     * Solve logistic regression by SVRG.
     * Objective function is the mean of 
     * f_j (w) = log (1 + exp(-z_j)) + <g_local - g_full, w> + 0.5*gamma*||w||_2^2, 
     * where z_j = <x_j, w>.
     *
     * @param x d-by-s feature matrix
     * @param gamma regularization parameter
     * @param learningrate learning rate (step size) of gradient descent
     * @param maxiter maximum number of iterations
     * @param gDiff local gradient minus full gradient
     * @param wArray initial w
     * @return w the trained model
     */
    def daneSvrgSolver(x: DenseMatrix[Double], gamma: Double, learningrate: Double, maxiter: Int = 20, gDiff: DenseVector[Double], wArray: Array[Double]): Array[Double] = {
        val s: Int = x.cols
        val d: Int = x.rows
        val invS: Double = -1.0 / s
        val sizeBatch: Int = 128
